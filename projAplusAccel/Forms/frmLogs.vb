...
Public Class frmLogs
    ...
    Sub EntHead()
        ...
        LstEmpRec.Columns.Add("ID Log", 100, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("IDnumber", 100, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("Name", 350, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("Shift", 170, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("TimeIn", 170, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("Status", 170, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("StatusTime", 170, HorizontalAlignment.Left)
    ...
    Sub AttHead()
        ...
        LstEmpRec.Columns.Add("Status", 170, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("StatusTime", 170, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("TotalTime", 170, HorizontalAlignment.Left)
        LstEmpRec.Columns.Add("", 0, HorizontalAlignment.Left)
    End Sub
    ...
    Sub EntSrch()
        ...
        If cboShift.Text = "All" Then
            strshift = vbNullString
        Else
            strshift = cboShift.Text
        End If
        ...
        'Dim rs As New Recordset
        rsPRS = New Recordset
        rsPRS = conSql.Execute("select * from tbltimein where Name like '" & txtInput.Text & "%' and TimeIn >= '" & TmeInFrm & "' and ShiftName like '" & strshift & "%' and TimeIn <='" & TmeInTo & "'")
        If rsPRS.EOF = False Then
            ProgBar.Value = 0
    ...
    Sub ExtSrch()
        ...
        'Dim rs As New Recordset
        rsOut = New Recordset
        rsOut = conSql.Execute("select * from tblTImeout where Name like '" & txtInput.Text & "%' and TimeOut >= '" & TmeInFrm & "' and TimeOut <='" & TmeInTo & "'")
        If rsOut.EOF = False Then
            ProgBar.Value = 0
    ...
    Sub InOutSrch()
        ...
        'Dim rs As New Recordset
        rsIO = New Recordset
        rsIO = conSql.Execute("select * from tblInOut where Name like '" & txtInput.Text & "%' and ShiftName like '" & strshift & "%' and TimeOut >= '" & TmeInFrm & "' and TimeOut <= '" & TmeInTo & "'")
        ...
        Select Case cboCateg.Text
            Case "Time In Logs"
                ...
                For Each viewlst In LstEmpRec.Items
                    If viewlst.Selected = True Then
                        Try
                            Dim Picname As String = Dir(My.Settings.CapImgPath & "\" & viewlst.SubItems(1).Text & "\" & viewlst.SubItems(4).Text & "\JPEGCapture\*.jpeg")
                            EntPic.ImageLocation = My.Settings.CapImgPath & "\" & viewlst.SubItems(1).Text & "\" & viewlst.SubItems(4).Text & "\JPEGCapture\" & Picname
                            EntPic.Load()
                        Catch ex As Exception
                            EntPic.Image = Image.FromFile(Application.StartupPath & "\noimage.jpeg")
                        End Try
            ...
            Case "Time Out Logs"
                ...
                For Each viewlst In LstEmpRec.Items
                    If viewlst.Selected = True Then
                        Try
                            Dim Picname As String = Dir(My.Settings.CapImgPath & "\" & viewlst.SubItems(4).Text & "\JPEGCapture\*.jpeg")
                            ExtPic.ImageLocation = My.Settings.CapImgPath & "\" & viewlst.SubItems(4).Text & "\JPEGCapture\" & Picname
                            ExtPic.Load()
                        Catch ex As Exception
                            ExtPic.Image = Image.FromFile(Application.StartupPath & "\noimage.jpeg")
                        End Try
            ...
            Case "Time In and Out Logs"
                ...
                For Each viewlst In LstEmpRec.Items
                    If viewlst.Selected = True Then
                        Try
                            Dim Picname As String = Dir(My.Settings.CapImgPath & "\" & viewlst.SubItems(5).Text & "\JPEGCapture\*.jpeg")
                            ExtPic.ImageLocation = My.Settings.CapImgPath & "\" & viewlst.SubItems(5).Text & "\JPEGCapture\" & Picname
                            ExtPic.Load()
                        Catch ex As Exception
                            ExtPic.Image = Image.FromFile(Application.StartupPath & "\noimage.jpeg")
                        End Try
    ...
    Private Sub cboShift_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cboShift.SelectedIndexChanged
