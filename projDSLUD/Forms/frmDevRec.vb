...
Public Class frmDevRec
    ...
    Sub Findfill()
        ...
        Dim lup As Short
        rs = New Recordset
        rs = PathConn.Execute("select * from tblDevices where " & cboCateg.Text & " like '" & txtInput.Text & "%' order by ID")
        If rs.EOF = False Then
            For lup = 1 To rs.RecordCount
                On Error Resume Next
                lstList.Refresh()
        ...
        For Each viewlst In lstList.Items
            If viewlst.Selected = True Then
                With frmAddDev
                    ...
                    .cboStatus.Text = viewlst.SubItems(4).Text
                    .txtDelay.Value = CInt(viewlst.SubItems(5).Text)
                    .ShowDialog()
                End With
            End If
        Next
